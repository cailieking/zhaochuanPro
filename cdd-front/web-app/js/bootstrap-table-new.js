!function ($) {
    ...
    BootstrapTable.DEFAULTS = {
        ...
        formatShowingRows: function (pageFrom, pageTo, totalRows) {
            return sprintf('显示 %s 到 %s 条记录，共 %s 条记录 ', pageFrom, pageTo, totalRows);
        },
            ...
            '<div class="bootstrap-table">',
                ...
                '<div class="fixed-table-container">',
                    ...
                    '<div class="fixed-table-body">',
                        ...
                        '</div>',
                    '</div>',
                    '<div class="fixed-table-pagination" style="width:1198px;margin:0 auto"></div>',
                '</div>',
            '</div>'].join(''));
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.showRefresh) {
            html.push(sprintf('<button class="btn btn-default" type="button" name="refresh" title="%s">',
                ...
                '<i class="glyphicon glyphicon-refresh fa fa-refresh table-icon"></i>',
                '</button>');
        }
        ...
        if (this.options.showToggle) {
            html.push(sprintf('<button class="btn btn-default" type="button" name="toggle" title="%s">',
                ...
                '<i class="glyphicon glyphicon glyphicon-list-alt fa fa-list-alt table-icon"></i>',
                '</button>');
        }
        ...
        if (this.options.showColumns) {
            html.push(sprintf('<div class="keep-open %s" title="%s">',
                ...
                '<button type="button" class="btn btn-default dropdown-toggle table-selbtn" data-toggle="dropdown">',
        ...
        if (this.options.search) {
            ...
            html.push(
                '<div class="pull-' + this.options.searchAlign + ' search">',
                    ...
                    sprintf('<label><input type="search" class="datatable-search-input form-control input-xs" placeholder="%s" ></label>',
                        this.options.formatSearch()),
                '<a href="javascript:;" class="datatable-search-btn button button-glow button-rounded button-highlight">查询</a>');

            this.$toolbar.append(html.join(''));
            $search = this.$toolbar.find('.search .datatable-search-btn');
            $search.off('click').on('click', function (event) {
                    that.onSearch(event);
            });
            $(".datatable-search-input").val(decodeURI(this.options.keyword)).keydown(function(event){
            	if(event.which == 13) {
            		that.onSearch(event);
//            		$(".datatable-search-btn").click();
    ...
    BootstrapTable.prototype.onSearch = function (event) {
        var text = $.trim(this.$toolbar.find('.search .datatable-search-input').val());

        // trim search input
        this.$toolbar.find('.search .datatable-search-input').val(text)
...
//        }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        html.push('</div>',
            '<div class="pull-right pagination" style="margin-top:0;">',
                '<ul class="pagination">',
        ...
        html.push(
                    ...
                    '<li class="page-last"><a href="javascript:void(0)">末页</a></li>',
                '</ul>',
            '</div>');
    ...
    BootstrapTable.prototype.initServer = function (silent) {
        ...
        if (this.options.keyword) {
        	data.search = decodeURI(this.options.keyword);
        }
...
//        this.initServer(params && params.silent);
    	this.initSearch();
    	this.onSearch(params.originalEvent)
    };
