!function ($) {
    ...
    BootstrapTable.prototype.initToolbar = function () {
        ...
        if (this.options.search) {
            ...
            $(".datatable-search-input").keydown(function(event){
            	if(event.which == 13) {
            		that.onSearch(event);
...
//        }
    ...
    BootstrapTable.prototype.initPagination = function () {
        ...
        $.each(pageList, function (i, page) {
            if (!that.options.smartDisplay || 101 >= page || i === 0) {
...
//        }
        html.push('<li class="page-number">',
        		'<input class="numberTextbox" type="text" size="5" />',
        '</li>');
        ...
        $number = this.$pagination.find('input');
        $number.val(this.options.pageNumber)
        var thisObj = this;
        $number.keydown(function(event) {
        	...
        	if(event.which == 13) {
        		thisObj.options.pageNumber = this.value
//        		$.proxy(thisObj.onPageNumber, thisObj);
        		thisObj.updatePagination(event);
        	}
        });
