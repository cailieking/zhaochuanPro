$(function(){
	checkClickEvent();//批次管理各个复选框的点击事件关联

	//setCommonPage2Event($('#shipPage2'),11,function(num){//初始化翻页控件 common.js中定义
		//zcAlert('批次管理'+num+'页');
	//})
	
	//初始化翻页控件 common.js中定义 
	var maxNum=parseInt($('#shipPage2').attr('data-maxSum')); 
	var pageSize=5;
	var maxPage=0;
	...
	setCommonPage2Event($('#shipPage2'),maxPage,function(num){
		window.location.href='/tariffManager/bactchList?page='+num;
	})
	var page=GetQueryString("page");
	if(page !=null && page.toString().length>0)
	{
		//console.log('page',page);
		commonPage2Change($('#shipPage2'),page,maxPage);
	}
...
'</div>';
 ...
 //运价发布
function shipCreate(){ 
	window.location.href = "/tariffManager/add";
}
...
function shipRepeal(id){
	zcConfirm("确定需要撤销吗？",function(m){
		 if(m){
			 window.location.href='/tariffManager/bactchRepeal?id=' + id;
		 }
	 })
}

function shipDelete(id){
	zcConfirm("确定需要删除吗？",function(m){
		 if(m){
			 window.location.href='/tariffManager/bactchDelete?id=' + id;
		 }
	 })
...
function batchSearch(){
	...
	var ends = $("#ends").val()
	var shipCompanysd = $('#shipCompanysd').val()
	var companys = $("#companys").val()  
	var createmans = $("#createmans").val();
	console.log(createmans)
	var createtimesd = $("#createtimesd").val()
	var releaseNums = $("#releaseNums").val()
	 //console.log(createtimesd);

	//window.location.href='/tariffManager/bactchList?starts='+starts+'&status='+status
	
	window.location.href='/tariffManager/bactchList?statusd='+statusd+'&starts='+starts+'&ends='+ends+'&shipCompanysd='+shipCompanysd+'&companys='+companys+'&createmans='+createmans+'&createtimesd='+createtimesd+'&releaseNums='+releaseNums
}
...
function bactchEditShip(key){ 
	...
	var doHtml= 
		 '<form action="/tariffManager/bactchSave"  method="post" id="dataFormCreats" ><input type="hidden" name="id" value="'+id+'" >'+
