...
$(function() {
	$('#list').bootstrapTable({
		url : '${request.contextPath}/endPort/list?dataType=json&requestDate='+new Date(),
	...
	addBtn.click(function() {
		window.location.href = '${request.contextPath}/endPort/edit/new';
	})
	...
	deleteBtn.click(function() {
		if(selectedDatas.length > 0) {
			BootstrapDialog.confirm("确定要删除选定记录吗？", function(result) {
				if(result) {
					window.location.href = '${request.contextPath}/endPort/delete/?ids=' + selectedDatas.join();
				}
			});
		} else {
...
window.operateEvents = {
		'click .edit' : function(e, value, row, index) {
			window.location.href = '${request.contextPath}/endPort/data/' + row.id;
		},
		'click .certificate' : function(e, value, row, index) {
		 
			window.location.href = '${request.contextPath}/endPort/edit/' + row.id;
		},
	};
