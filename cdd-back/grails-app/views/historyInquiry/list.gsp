...
<asset:stylesheet src="c_css/inquiry_history.css" />
...
function initInquirys(){
	...
	$.ajax({
		url:  "/historyInquiry/listInquirys",
		type: "post",
		dataType: 'json',
		data: searchArgs,
		success:function(rs){
    			...
    			'<th width="80">商务专员</th>'+
    			'<th width="85">日期</th>'+
    			'<th width="80">时间</th><th width="125">操作</th></tr></thead><tr style="height:10px;border:0;"></tr><tbody>'
	       		...
	       		for(var i in rows){
	        		...
	        		var dateTime = date.getFullYear()
              		...
              		var time = 
               		...
               		tb += '<td>'+dateTime+'</td>'+
	        			'<td>'+time+'</td>'+
	        			'<td><span class="oper" data-number="'+rows[i][0]+'" onClick="inquirySuppl(event)">补充</span><span class="oper" data-number="'+rows[i][0]+'" onClick="inquiryStop(event)">终结</span></td>'+
	        		'</tr>'
	       		}	
...
function inquiryHistory(eve){
		...
		$.ajax({
				type:'post',
				url:'/historyInquiry/detailInquiry',
				dataType:'json',
