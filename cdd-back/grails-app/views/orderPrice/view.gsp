...
<%--<div class="row">
	--%><div class="col-lg-12">
		...
		<div class="layui-layer-title" style="cursor: move;border-radius:5px" move="ok">运价信息</div>
			<form action="${request.contextPath }/orderPrice/operate" method="post" id="dataForm">
			<input type="hidden" name="id" value="${data.data.id}" />
			<table>
			 	<tr class="only-tr">
					<td>货盘编号</td><td   title='货盘编号'>
						<input type="text" readonly="readonly" value="${data.data.order.number}" />
					</td >
				</tr>
				<tr class="only-tr">
				...
				<tr class="only-tr">
					<td>备注</td>
					<td title="备注" colspan="3"><input type="text" readonly="readonly" value="${data.data.order.remark }"/></td>
					<td></td>
					<td></td>
				</tr>
				...
				<g:if test="${data.data.info!=null }">
					...
					<tr class="only-tr">
						<td>运输类别</td>
						<td title="运输类别">
							<input type="text" name="transType" readonly="readonly" value="${data.data.info.transType?.text }"/>
						</td>
						<td>有效期</td>
						<td title="有效期">
							<input type="text" name="youxiaoqi" readonly="readonly" value="${data.data.info.startDate ? dateFormat.format(data.data.info.startDate): ''  }至${data.info.endDate ? dateFormat.format(data.info.endDate): ''  }"/>
						</td>
						<td>限重</td>
						<td title="限重">
			...
			<div class="buttons">
			<a href="javascript:;" onclick="$('#dataForm').submit()" class="button button-glow button-rounded button-raised button-primary pull-right">受理</a>
