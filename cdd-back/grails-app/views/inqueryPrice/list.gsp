...
$(function() {
	...
	$('#list').bootstrapTable({
		url : '${request.contextPath}/inqueryPrice/list?dataType=json&requestDate='+new Date(),
		sidePagination : 'server', // client or server
		formatSearch : function() {
			return '询价编号/公司名称/电话号码';
		},
		columns : [ {
			checkbox : true
		}, {
			field : 'number',
			title : '询价编号',
			formatter : function (value, row, index) {<%--href="/inqueryPrice/view/'+row.id+'" target="_blank"--%>
				return [
			...
			formatter : function (value, row, index) {<%-- href="/shipInfo/data/'+value+'" target="_blank" --%>
				return [
						'<a class="datatable_operation yjview" href="javascript:;" >',
						value, 
						'</a>' 
					].join('');
				},
	...
	//	},
		...
		//  events: operateEvents
			...
			field : 'id',
			title : '操作',
			formatter : function (value, row, index) {<%--href="/inqueryPrice/update/'+value+'" target="_blank" --%>
				return [
						'<a class="datatable_operation update" href="javasctipt:void(0);">',
						'修改', 
						'</a>',
	...
	deleteBtn.click(function() {
		if(selectedDatas.length > 0) {
			BootstrapDialog.confirm("确定要删除选定记录吗？", function(result) {
				if(result) {
					window.location.href = '${request.contextPath}/inqueryPrice/delete/?ids=' + selectedDatas.join();
				}
			});
		} else {
...
window.operateEvents = {
	'click .operate' : function(e, value, row, index) {
		if(row.status=="已受理"){
			BootstrapDialog.show({
				...
				window.location.href = '${request.contextPath}/inqueryPrice/operate/' + value;
			}
	...
	'click .finish' : function(e, value, row, index) {
		if(row.status=="未受理"){
			BootstrapDialog.show({
				...
				window.open('${request.contextPath}/inqueryPrice/finish/' + value);
	...
	'click .view' : function(e, value, row, index) {
		value=row.id;
		window.location.href = '${request.contextPath}/inqueryPrice/view/' + value;
	},
	'click .update' : function(e, value, row, index){
		
			window.location.href = '${request.contextPath}/inqueryPrice/update/' + value;
	},
	'click .yjview' : function(e,value,row,index){
			window.location.href='${request.contextPath}/shipInfo/data/' +value;
	},
