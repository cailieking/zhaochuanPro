...
class AllInquiryController extends BaseController{

   def model = "/allInquiry"
	...
	def listInquirys(){
		...
		sb.append("SELECT count(*) FROM inquiry_offer_price AS a LEFT JOIN back_user AS b ON a.create_by = b.username LEFT JOIN end_port AS c ON a.end_port=c.port_english_name LEFT JOIN new_route AS d ON c.route_id  = d.id WHERE 1+1 ")
		...
		if(!"全部".equals(params.dealName)){
			//def u = BackUser.findByFirstname(params.dealName)
			sb.append(" and b.firstname = '" + params.dealName + "'")
		}
		
		if(!"全部".equals(params.routeName)){
			sb.append(" and d.route_name = '" + params.routeName + "'")
		}
...
//						}
						...
						if(params.timeSlot){
							switch(params.timeSlot){
								case "今天" : sb1.append(" and DATE_FORMAT(a.date_created, '%Y-%m-%d') = '" + sdf.format(d) +"' ")
									break;
								case "昨天" :calendar.setTime(d)
									calendar.add(calendar.DATE,-1)
									Date yt = calendar.getTime()
						 ...
						 if(!"全部".equals(params.dealName)){
							 sb1.append(" and b.firstname = '" + params.dealName +"'")
						 }
						 
						 if(!"全部".equals(params.routeName)){
							 sb1.append(" and d.route_name = '" + params.routeName+"'")
						 }
	...
	def countByStatus(String pId,BackUser user){
		...
		StringBuffer sb = new StringBuffer()
		sb.append("select inquiry_status,count(inquiry_status)  from inquiry_offer_price as a left join back_user as b ON a.create_by = b.username  where 1+1 ")
